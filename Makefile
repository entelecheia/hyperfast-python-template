...
init-project: initialize ## initialize the project (Warning: do this only once!)
	@copier copy --trust --answers-file .copier/.copier-config.yaml --vcs-ref=HEAD gh:entelecheia/hyperfast-python-template .

reinit-project: install-copier ## reinitialize the project (Warning: this may overwrite existing files!)
	@bash -c 'args=(); while IFS= read -r file; do args+=("--skip" "$$file"); done < .copier/.copierignore; copier copy "$${args[@]}" --answers-file .copier/.copier-config.yaml --trust --vcs-ref=HEAD . .'
...
reinit-project-force: install-copier ## initialize the project ignoring existing files (Warning: this will overwrite existing files!)
	@bash -c 'args=(); while IFS= read -r file; do args+=("--skip" "$$file"); done < .copier/.copierignore; copier copy "$${args[@]}" --answers-file .copier/.copier-config.yaml --trust --force --vcs-ref=HEAD . .'

test-init-project: install-copier ## test initializing the project to a temporary directory
	@bash -c 'args=(); while IFS= read -r file; do args+=("--skip" "$$file"); done < .copier/.copierignore; copier copy "$${args[@]}" --answers-file .copier/.copier-config.yaml --trust --vcs-ref=HEAD . tmp'
	@rm -rf tmp/.git

test-init-project-force: install-copier ## test initializing the project to a temporary directory forcing overwrite
	@bash -c 'args=(); while IFS= read -r file; do args+=("--skip" "$$file"); done < .copier/.copierignore; copier copy "$${args[@]}" --answers-file .copier/.copier-config.yaml --trust --force --vcs-ref=HEAD . tmp'
	@rm -rf tmp/.git
